#!/usr/bin/env python3
"""
Reproduce 365-day backtest for Asia ORB candidates 47-48 ONLY
Compare computed metrics vs target metrics from testfix2.txt
"""
...
TARGET_METRICS = {
    47: {"trades": 257, "win_rate": 0.529, "avg_r": 0.055, "total_r": 14.1},
    48: {"trades": 257, "win_rate": 0.354, "avg_r": 0.054, "total_r": 13.9},
}

# Tolerance (from testfix2.txt)
TOLERANCE = {
...
def main():
    ...
    print("="*80)
    print("VERIFY ASIA CANDIDATES 47-48: 365-DAY BACKTEST")
    print("="*80)
    print()
    ...
    # Run backtests for candidates 47 and 48 only
    results = []
    ...
    lines = []
    lines.append("# VERIFY ASIA CANDIDATES 47-48: 365-DAY BACKTEST")
    lines.append("")
    ...
    if all_passed:
        ...
        lines.append("")
        lines.append("Both candidates passed verification. Metrics match target within tolerance.")
    else:
    ...
    # Save report
    output_path = root / "research" / "quick_asia" / "VERIFY_ASIA_47_48.md"
    ...
    if not all_passed:
        ...
        print("Creating FAIL report as per testfix2.txt instructions...")

        # Create FAIL report
        fail_lines = []
        fail_lines.append("# VERIFY ASIA CANDIDATES 47-48: FAIL REPORT")
        fail_lines.append("")
        fail_lines.append(f"**Date**: {date.today().isoformat()}")
        fail_lines.append(f"**Status**: VERIFICATION FAILED")
        fail_lines.append("")
        fail_lines.append("## Failure Summary")
        fail_lines.append("")
        ...
        for vr in verification_results:
            if not vr["all_pass"]:
                ...
                fail_lines.append(f"### Candidate {cid}: {cfg['orb_time']} ORB RR={cfg['rr']} {cfg['sl_mode']}")
                fail_lines.append("")
                fail_lines.append("**Mismatched Metrics**:")

                if not vr["trades_ok"]:
                    fail_lines.append(f"- Trades: Computed={comp['trades']}, Target={tgt['trades']} (FAIL)")
                if not vr["win_rate_ok"]:
                    fail_lines.append(f"- Win Rate: Computed={comp['win_rate']:.3f}, Target={tgt['win_rate']:.3f} (FAIL)")
                if not vr["avg_r_ok"]:
                    fail_lines.append(f"- Avg R: Computed={comp['avg_r']:.3f}, Target={tgt['avg_r']:.3f} (FAIL)")
                if not vr["total_r_ok"]:
                    fail_lines.append(f"- Total R: Computed={comp['total_r']:.1f}, Target={tgt['total_r']:.1f} (FAIL)")

                fail_lines.append("")

        fail_lines.append("## Root Cause")
        fail_lines.append("")
        fail_lines.append("To be investigated. Possible causes:")
        fail_lines.append("- Target metrics incorrect")
        fail_lines.append("- Backtest engine logic mismatch")
        fail_lines.append("- Data inconsistency")
        fail_lines.append("")

        fail_path = root / "research" / "quick_asia" / "VERIFY_ASIA_47_48_FAIL.md"
        with open(fail_path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(fail_lines))

        print(f"FAIL report saved to: {fail_path}")
        sys.exit(1)
    else:
