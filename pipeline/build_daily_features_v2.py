...
# ==============================================================================
# TRANSACTION COSTS (Single Source of Truth - Configured Here Only)
# ==============================================================================
# MGC Micro Gold futures transaction costs
# IMPORTANT: Adjust SLIPPAGE_TICKS based on your actual execution quality
# Conservative: 0.5 ticks | Moderate: 1.0 ticks | Worst-case: 2.0 ticks
COMMISSION_RT = 2.0  # Round-trip commission in dollars (entry + exit)
SLIPPAGE_TICKS = 1.0  # Total slippage in ticks for round-trip (MODERATE scenario)
TICK_VALUE = 1.00  # MGC tick value in dollars ($1.00 per tick)
...
def apply_costs(r_multiple_gross: Optional[float], risk_ticks: float) -> Optional[float]:
    ...
    Args:
        r_multiple_gross: Gross R multiple (before costs)
        risk_ticks: Risk in ticks (ORB size for FULL mode, half ORB for HALF mode)

    Returns:
        Net R multiple (after costs), or None if r_multiple_gross is None
    ...
    Example:
        Gross R = +2.0, Risk = 2.5 ticks ($2.50)
        Cost = $2.50 / $2.50 = 1.0R
        Net R = +2.0 - 1.0 = +1.0R (still profitable)

        Gross R = +1.0, Risk = 2.5 ticks ($2.50)
        Cost = $2.50 / $2.50 = 1.0R
        Net R = +1.0 - 1.0 = 0.0R (breakeven)

        Gross R = +1.0, Risk = 10.0 ticks ($10.00)
        Cost = $2.50 / $10.00 = 0.25R
        Net R = +1.0 - 0.25 = +0.75R (profitable)
    """
    # Edge case: NO_TRADE or missing data
    if r_multiple_gross is None:
        return None

    # Safety guard: risk must be positive
    if risk_ticks is None or risk_ticks <= 0:
        # If risk is zero, can't normalize costs - return gross R
        return r_multiple_gross

    # Calculate total cost in dollars
    # SLIPPAGE_TICKS already represents total round-trip slippage (not per fill)
    cost_dollars = COMMISSION_RT + (SLIPPAGE_TICKS * TICK_VALUE)

    # Risk in dollars
    risk_dollars = risk_ticks * TICK_VALUE

    # Cost in R terms (normalized by risk)
...
class FeatureBuilderV2:
    ...
    def calculate_orb_1m_exec(self, orb_start_local: datetime, scan_end_local: datetime, rr: float = RR_DEFAULT, sl_mode: str = SL_MODE) -> Optional[Dict]:
        ...
        Returns:
            Dict with keys: high, low, size, break_dir,
                           outcome (gross), r_multiple (gross),
                           outcome_net, r_multiple_net,
                           mae, mfe, stop_price, risk_ticks
        """
        ...
        if break_dir == "NONE":
            return {
                ...
                "break_dir": "NONE",
                "outcome": "NO_TRADE",  # gross outcome
                "r_multiple": None,  # gross R
                "outcome_net": "NO_TRADE",  # net outcome (same, no costs for no trade)
                "r_multiple_net": None,  # net R (same, no costs for no trade)
                "mae": None, "mfe": None,
        ...
        if r_orb <= 0:
            ...
            return {
                "high": orb_high, "low": orb_low, "size": orb_size,
                "break_dir": break_dir,
                "outcome": "NO_TRADE",  # gross
                "r_multiple": None,  # gross
                "outcome_net": "NO_TRADE",  # net (same, no trade executed)
                "r_multiple_net": None,  # net
                "mae": None, "mfe": None,
        ...
        for ts_utc, h, l, c in bars[start_i:]:
            ...
            if break_dir == "UP":
                ...
                if hit_stop and hit_target:
                    # Both hit in same bar - conservative: LOSS
                    r_multiple_gross = -1.0
                    r_multiple_net = apply_costs(r_multiple_gross, risk_ticks)
                    outcome_net = "LOSS" if r_multiple_net and r_multiple_net < 0 else "WIN"
                    return {
                        "high": orb_high, "low": orb_low, "size": orb_size,
                        "break_dir": break_dir,
                        "outcome": "LOSS",  # gross (conservative: stop first)
                        "r_multiple": r_multiple_gross,  # gross
                        "outcome_net": outcome_net,  # net (after costs)
                        "r_multiple_net": r_multiple_net,  # net (after costs)
                        "mae": mae_raw / r_orb, "mfe": mfe_raw / r_orb,
                        "stop_price": stop, "risk_ticks": risk_ticks
                    }
                if hit_target:
                    # Target hit - WIN (gross)
                    r_multiple_gross = float(rr)
                    r_multiple_net = apply_costs(r_multiple_gross, risk_ticks)
                    outcome_net = "WIN" if r_multiple_net and r_multiple_net > 0 else "LOSS"
                    return {
                        "high": orb_high, "low": orb_low, "size": orb_size,
                        "break_dir": break_dir,
                        "outcome": "WIN",  # gross
                        "r_multiple": r_multiple_gross,  # gross
                        "outcome_net": outcome_net,  # net (could flip to LOSS if costs > profit!)
                        "r_multiple_net": r_multiple_net,  # net
                        "mae": mae_raw / r_orb, "mfe": mfe_raw / r_orb,
                        "stop_price": stop, "risk_ticks": risk_ticks
                    }
                if hit_stop:
                    ...
                    return {
                        ...
                        "outcome": "LOSS",  # gross
                        "r_multiple": r_multiple_gross,  # gross
                        "outcome_net": outcome_net,  # net (will be more negative due to costs)
                        "r_multiple_net": r_multiple_net,  # net
                        "mae": mae_raw / r_orb, "mfe": mfe_raw / r_orb,
                        "stop_price": stop, "risk_ticks": risk_ticks
                    }
        ...
        # No exit (timeout - neither stop nor target hit)
        return {
            "high": orb_high, "low": orb_low, "size": orb_size,
            "break_dir": break_dir,
            "outcome": "NO_TRADE",  # gross (no exit)
            "r_multiple": None,  # gross
            "outcome_net": "NO_TRADE",  # net (same - no exit, no P/L, no costs)
            "r_multiple_net": None,  # net
    ...
    def build_features(self, trade_date: date) -> bool:
        ...
        self.con.execute(
            ...
            [
                ...
                orb_0900.get("outcome_net") if orb_0900 else None,
                orb_0900.get("r_multiple_net") if orb_0900 else None,
                orb_1000.get("outcome_net") if orb_1000 else None,
                orb_1000.get("r_multiple_net") if orb_1000 else None,
                orb_1100.get("outcome_net") if orb_1100 else None,
                orb_1100.get("r_multiple_net") if orb_1100 else None,
                orb_1800.get("outcome_net") if orb_1800 else None,
                orb_1800.get("r_multiple_net") if orb_1800 else None,
                orb_2300.get("outcome_net") if orb_2300 else None,
                orb_2300.get("r_multiple_net") if orb_2300 else None,
                orb_0030.get("outcome_net") if orb_0030 else None,
                orb_0030.get("r_multiple_net") if orb_0030 else None,
            ],
        )
    ...
    def init_schema_v2(self):
        self.con.execute(
            ...
            CREATE TABLE IF NOT EXISTS {self.table_name} (
                ...
                -- NET PERFORMANCE (After Transaction Costs)
                -- Added 2026-01-24: Commission + Slippage applied to R multiples
                orb_0900_outcome_net VARCHAR,
                orb_0900_r_multiple_net DOUBLE,
                orb_1000_outcome_net VARCHAR,
                orb_1000_r_multiple_net DOUBLE,
                orb_1100_outcome_net VARCHAR,
                orb_1100_r_multiple_net DOUBLE,
                orb_1800_outcome_net VARCHAR,
                orb_1800_r_multiple_net DOUBLE,
                orb_2300_outcome_net VARCHAR,
                orb_2300_r_multiple_net DOUBLE,
                orb_0030_outcome_net VARCHAR,
                orb_0030_r_multiple_net DOUBLE,
