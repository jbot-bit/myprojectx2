...
def test_ml_integration():
    ...
    loader.refresh()
    print(f"  Loaded {len(loader.bars_df)} 1-minute bars")
    ...
    if not ml_found and evaluation.state.value in ["PREPARING", "READY"]:
        print(f"\n[WARN]  No ML insight in reasons (strategy not in right state)")
    elif not ml_found:
        print(f"\n[WARN]  No ML insight (expected for {evaluation.state.value} state)")
